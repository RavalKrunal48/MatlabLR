...
% compute error per basis function
% Find support of each function on the element
for ifn = 1:size(lr.cp, 2)
  el_index = [];
  for iel = 1:size(lr.elements,1)
    el_ind = find([lr.support{iel,:}] == ifn);
    if ~isempty(el_ind)
      el_index(end+1) = iel;
    end
  end
function_support{ifn,1} = el_index;
end
% function_support

function_velocity_error_H1_squared = zeros(size(function_support,1),1);
for ifn = 1:size(function_support)
  function_velocity_error_H1_squared(ifn) = sum(velocity_error_H1_squared(function_support{ifn}));
end
function_velocity_error_H1_squared;
